...
public record SoftMalletHeadMaterialStats(int durability, float miningSpeed, Tier tier, float attack) implements IRepairableMaterialStats {
    public static final MaterialStatsId ID = new MaterialStatsId(GTConstruct.id("soft_mallet_head"));

    public static final MaterialStatType<SoftMalletHeadMaterialStats> TYPE = new MaterialStatType<>(ID, new SoftMalletHeadMaterialStats(1, 1f, Tiers.WOOD, 1f), RecordLoadable.create(
            IRepairableMaterialStats.DURABILITY_FIELD,
            FloatLoadable.FROM_ZERO.defaultField("mining_speed", 1f, true, SoftMalletHeadMaterialStats::miningSpeed),
            TinkerLoadables.TIER.defaultField("mining_tier", Tiers.WOOD, true, SoftMalletHeadMaterialStats::tier),
            FloatLoadable.FROM_ZERO.defaultField("melee_attack", 1f, true, SoftMalletHeadMaterialStats::attack),
            SoftMalletHeadMaterialStats::new));
